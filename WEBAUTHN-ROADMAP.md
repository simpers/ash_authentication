...
- [ ] Choose underlying WebAuthn lib (`wax_` library)
- [ ] Add a narrow boundary module, `AshAuthentication.WebAuthn.Adapter` behaviour
- [ ] Provide a default adapter implementation (`WaxAdapter`)
- [ ] Establish how we represent and validate:
  - [ ] rpId (via DSL `rp_id` option)
  - [ ] origin (via DSL `origin` option passed to adapter)
  - [ ] challenge (raw bytes generated by wax_)
  - [ ] user handle / discoverable credentials (deferred to later PR)
- [ ] Add unit tests with fixtures and/or adapter mocks
- [ ] Add compile-time checks for `:cbor` and `:wax_` dependencies
